...
사실을 세 가지 방법으로 뽑아 합친다.

- **규칙**: 위키 문서 하단에 붙는 "분류" 태그를 정규식으로 읽는다.
  예를 들어 어떤 그룹 문서에 "SM 엔터테인먼트 소속" 분류가 달려
  있으면 `(그 그룹, SIGNED_TO, SM 엔터테인먼트)`로, "2006년 결성된
  음악 그룹" 분류가 달려 있으면 `(그 그룹, FORMED_IN, 2000s)`로
  바로 변환한다. 사람이 정리해 둔 분류를 읽는 것이라 틀릴 일이
  거의 없고 LLM 비용도 안 든다 — 이런 규칙을 10개 만들었다.
- **트랙리스트 정규식**: 음반 문서의 "수록곡" 목록("1. 곡 제목",
  "2. 곡 제목" 같은 번호 매김 줄)을 그대로 파싱해 `(음반, CONTAINS,
  곡)` 관계를 뽑는다. 이것도 규칙과 마찬가지로 확실한 사실이다.
- **LLM 추출**: 분류 태그나 번호 목록으로 안 잡히는 나머지 문장
  (예: "이 곡은 OOO가 작사·작곡했다", "OOO의 영향을 받았다")에서
  LLM이 관계를 읽어낸다. 세 방법 중 유일하게 틀릴 가능성이 있어서,
  뽑아낸 문장이 실제 원문에 있는지 전부 대조해 88.2%만 채택한다
  (나머지는 원문에 없는, LLM이 지어낸 것으로 보고 버린다).

**그래프에 무엇을 담았나**: 개체(노드)는 9종류 — 가수(Artist), 그룹
(Group), 음반(Album), 곡(Song), 기획사(Label), 시상식(Award), 방송
