...
**지식그래프**(첫 탭으로 바로 보이게 배치. 드래그 가능한 인터랙티브
그래프, 노드 타입별 색상, 답변에 실제로 인용된 노드·엣지는 빨간
테두리/빨간 선으로 강조하고 나머지는 회색으로 옅게, 시드 노드는 화면
중앙에 고정), 순회 경로(홉별 화살표), 근거 삼중항 표, 원문 문서 발췌.
기권 시에는 그 사실과 미확보 관계를 그대로 보여준다 — 환각 방지 장치가
작동했음을 화면에서 확인할 수 있다.
...
**데모 개발 중 발견·수정한 실제 버그 3개**(모두 실행해 보고서야 드러난
것들이다): ① `streamlit_agraph.Config(width="100%")`가 내부에서
`f"{width}px"`로 조립되어 `"100%px"`라는 무효 CSS 값이 되고, 그 결과
노드 하나가 화면을 가득 채우는 버그 — 숫자 px로 고정해 해결. ②
시드가 아닌 노드에 `x=None, y=None`을 명시적으로 넘기면 그대로
`null`이 직렬화되어 vis.js 좌표가 `NaN`이 되고 그래프 전체가 빈
화면으로 보이는 버그 — 위치 kwargs를 시드 노드에만 준다. ③
`streamlit_agraph.agraph()`가 내부 컴포넌트의 `key` 인자를 흘려보내지
않아 채팅 턴이 여러 개면 `StreamlitDuplicateElementId`로 죽는 버그 —
`_agraph`를 직접 호출하는 로컬 래퍼로 우회.
