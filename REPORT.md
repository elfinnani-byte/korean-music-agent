...
| 홉별 생성 점수 — 이 에이전트(GraphRAG) | 0.500 / 0.889 / 0.714 |
| 홉별 생성 점수 — 기존 방식(Basic RAG/BM25) | 0.833 / 0.778 / 0.714 |
...
| | 기존 RAG (Basic RAG) | 이 에이전트 (GraphRAG) |
| --- | --- | --- |
...
25문항을 직접 만들었다(튜닝 15 + 한 번도 손대지 않은 홀드아웃 10).
문항마다 정답만 있는 게 아니라 **그래프에서 실제로 타야 할 경로**와
**그 경로가 근거로 삼을 정확한 사실**, 그리고 그 사실이 원문 어디에
적혀 있는지 인용문까지 갖췄다 — 정답을 맞혔는지뿐 아니라 "올바른
근거로" 맞혔는지까지 채점하기 위해서다.

예시(Q01, "동방신기는 어느 기획사에 소속되어 있는가?"):
- 정답: `SM 엔터테인먼트`(동의어: `SM`, `에스엠 엔터테인먼트`)
- 기대 경로: 동방신기 →(SIGNED_TO)→ SM 엔터테인먼트 (1홉)
- 원문 근거: `SM_엔터테인먼트.md`의 실제 인용문
...
| 홉 | 이 에이전트 | 기존 방식(Basic RAG) | 결과 |
| --- | --- | --- | --- |
| 1홉(단순 사실) | 0.500 | 0.833 | 기존 방식 우세 |
| 2홉(관계 2단계) | 0.889 | 0.778 | **이 에이전트 우세** |
| 3홉(관계 3단계) | 0.714 | 0.714 | 동률 |

가설("관계가 복잡할수록 그래프 방식이 유리하다")은 2홉에서는
맞았지만, 3홉은 동률에 그쳤다. 1홉은 오히려 기존 방식이 앞섰다 —
답이 원문에 그대로 있는 단순 질문은 그래프를 탈 필요가 없고,
오히려 허브 회피 규칙 때문에 손해를 본다.

**정확히 얼마나 신뢰할 수 있는 결과인가**: 튜닝에 쓴 15문항 평균은
0.733, 한 번도 안 본 홀드아웃 10문항 평균은 0.800 — 홀드아웃이 더
높게 나와 "튜닝 문항에만 맞춘" 징후는 없었다. 근거 재현율(그래프가
사실을 실제로 찾았는가)과 생성 점수(채점 결과)는 곳에 따라 크게
벌어진다(2홉: 0.093 대 0.889) — 채점 모델이 기권 응답도 "정답과
실질적으로 같다"며 후하게 매기는 경우가 섞여 있어서다. 그래서 점수
하나만 보지 않고, 그래프가 실제로 사실을 찾았는지까지 함께 본다.
